'use client';

import { useState } from 'react';

interface ValueChainNode {
  company: string;
  tier2?: ValueChainNode[];
}

interface ValueChainData {
  company: string;
  tier1: ValueChainNode[];
}

interface DbValueChainNode {
  company: string;
  parentCompany: string | null;
  tier: number;
}

const W = 1500;
const H = 680;
const AX = 750;
const AY = 340;
const AW = 210;
const AH = 74;
const CW = 185;
const CH = 30;
const T2_GAP = 44;
const MAX_VISIBLE_T1 = 5;
const MAX_VISIBLE_T2 = 3;

const T1_HUB_X = 530;
const CUST_HUB_X = 970;
const T1_CARD_RX = T1_HUB_X - 55;
const T1_CARD_LX = T1_CARD_RX - CW;
const T2_CARD_RX = T1_CARD_LX - 28;
const T2_CARD_LX = T2_CARD_RX - CW;
const CUST_CARD_LX = CUST_HUB_X + 55;

type NodeType = 'Supplier' | 'Local Customer' | 'Global Customer';

interface ChainNode {
  id: string;
  name: string;
  nodeType: NodeType;
  parentId?: string;
}

const CUSTOMERS: ChainNode[] = [
  { id: 'bestmeat', name: 'Best Meat', nodeType: 'Local Customer' },
  { id: 'fast', name: 'PT FAST (KFC)', nodeType: 'Local Customer' },
  { id: 'eka', name: 'PT Eka Bogainti', nodeType: 'Local Customer' },
  { id: 'magetan', name: 'PT Magetan Gajah Mada', nodeType: 'Local Customer' },
  { id: 'toh', name: 'Toh Thye San Farm', nodeType: 'Global Customer' },
  { id: 'yanur', name: 'Ya Nur Trading Co', nodeType: 'Global Customer' },
  { id: 'sojitz', name: 'Sojitz Foods Corp', nodeType: 'Global Customer' },
];

const TYPE_COLOR: Record<NodeType, string> = {
  'Supplier': '#1e3a6e',
  'Local Customer': '#dc2626',
  'Global Customer': '#0f2d52',
};

const TYPE_BADGE: Record<NodeType, string> = {
  'Supplier': 'Supplier',
  'Local Customer': 'Local',
  'Global Customer': 'Global',
};

function ySpread(count: number, margin: number): number[] {
  if (count <= 1) return [H / 2];
  const step = (H - margin * 2) / (count - 1);
  return Array.from({ length: count }, (_, i) => margin + i * step);
}

function generateId(name: string): string {
  return name.toLowerCase().replace(/[^a-z0-9]/g, '').substring(0, 20);
}

function convertDbToT1T2(nodes: DbValueChainNode[]): { t1: ChainNode[]; t2: ChainNode[] } {
  const t1Nodes = nodes.filter(n => n.tier === 1 && n.parentCompany === null).slice(0, MAX_VISIBLE_T1);
  const t2Nodes = nodes.filter(n => n.tier === 2);

  const t1: ChainNode[] = t1Nodes.map(n => ({
    id: generateId(n.company),
    name: n.company,
    nodeType: 'Supplier' as NodeType,
  }));

  const t2: ChainNode[] = [];
  for (const t1Node of t1Nodes) {
    const t1Id = generateId(t1Node.company);
    const children = t2Nodes.filter(n => n.parentCompany === t1Node.company).slice(0, MAX_VISIBLE_T2);
    for (const child of children) {
      t2.push({
        id: generateId(child.company),
        name: child.company,
        nodeType: 'Supplier' as NodeType,
        parentId: t1Id,
      });
    }
  }

  return { t1, t2 };
}

export default function ValueChainDynamic() {
  const [jsonInput, setJsonInput] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [hovered, setHovered] = useState<string | null>(null);
  const [dbLoading, setDbLoading] = useState(false);
  const [dbError, setDbError] = useState<string | null>(null);
  const [dbLoaded, setDbLoaded] = useState(false);

  const [t1Suppliers, setT1Suppliers] = useState<ChainNode[]>([]);
  const [t2Suppliers, setT2Suppliers] = useState<ChainNode[]>([]);
  const [anchorName, setAnchorName] = useState('PT JAPFA');

  const loadFromDb = async (accountId: string) => {
    setDbLoading(true);
    setDbError(null);
    setError(null);
    try {
      const res = await fetch(`/api/value-chain?accountId=${accountId}&clientType=Supplier`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const nodes: DbValueChainNode[] = await res.json();

      const { t1, t2 } = convertDbToT1T2(nodes);
      setT1Suppliers(t1);
      setT2Suppliers(t2);
      setDbLoaded(true);
    } catch (e) {
      setDbError(e instanceof Error ? e.message : 'Failed to load from DB');
    } finally {
      setDbLoading(false);
    }
  };

  const handleProcess = () => {
    setError(null);
    setDbLoaded(false);

    if (!jsonInput.trim()) {
      setError('Please enter JSON data');
      return;
    }

    try {
      const data: ValueChainData = JSON.parse(jsonInput);

      if (!data.company || !data.tier1) {
        setError('Invalid format: need { company, tier1[] }');
        return;
      }

      setAnchorName(data.company);

      const t1: ChainNode[] = data.tier1
        .slice(0, MAX_VISIBLE_T1)
        .map((item) => ({
          id: generateId(item.company),
          name: item.company,
          nodeType: 'Supplier' as NodeType,
        }));

      const t2: ChainNode[] = [];
      const maxTier1 = data.tier1.slice(0, MAX_VISIBLE_T1);
      for (const tier1Item of maxTier1) {
        const parentId = generateId(tier1Item.company);
        if (tier1Item.tier2) {
          const visibleTier2 = tier1Item.tier2.slice(0, MAX_VISIBLE_T2);
          for (const tier2Item of visibleTier2) {
            t2.push({
              id: generateId(tier2Item.company),
              name: tier2Item.company,
              nodeType: 'Supplier' as NodeType,
              parentId: parentId,
            });
          }
        }
      }

      setT1Suppliers(t1);
      setT2Suppliers(t2);
    } catch (e) {
      setError(`JSON parse error: ${e instanceof Error ? e.message : 'Unknown error'}`);
    }
  };

  const t1Ys = ySpread(Math.max(t1Suppliers.length, 1), 100);

  const t1YMap: Record<string, number> = {};
  t1Suppliers.forEach((n, i) => { t1YMap[n.id] = t1Ys[i]; });

  const t2Positions: { node: ChainNode; y: number }[] = [];
  t1Suppliers.forEach(t1 => {
    const children = t2Suppliers.filter(t2 => t2.parentId === t1.id);
    const parentY = t1YMap[t1.id] ?? AY;
    children.forEach((node, idx) => {
      const offset = (idx - (children.length - 1) / 2) * T2_GAP;
      t2Positions.push({ node, y: parentY + offset });
    });
  });

  const custYs = ySpread(CUSTOMERS.length, 80);

  function renderCard(node: ChainNode, lx: number, cy: number) {
    const isHov = hovered === node.id;
    const color = TYPE_COLOR[node.nodeType];
    const isDashed = node.nodeType === 'Global Customer';
    return (
      <g
        key={node.id}
        onMouseEnter={() => setHovered(node.id)}
        onMouseLeave={() => setHovered(null)}
        style={{ cursor: 'default' }}
      >
        <rect
          x={lx} y={cy - CH / 2} width={CW} height={CH} rx={3}
          fill={isHov ? '#eff6ff' : 'white'}
          stroke={isHov ? '#2563eb' : isDashed ? '#0f2d52' : '#e5e7eb'}
          strokeWidth={isHov ? 1.5 : 1}
          strokeDasharray={isDashed && !isHov ? '5 3' : undefined}
        />
        <rect x={lx} y={cy - CH / 2} width={4} height={CH} rx={2} fill={color} />
        <text x={lx + 12} y={cy + 4} fontSize={9.5}
          fontWeight={isHov ? '700' : '500'}
          fill={isHov ? '#1d4ed8' : '#1f2937'}
          pointerEvents="none">
          {node.name}
        </text>
        <text x={lx + CW - 8} y={cy + 4} fontSize={8} fontWeight="600"
          fill={isHov ? '#2563eb' : '#9ca3af'}
          textAnchor="end" pointerEvents="none">
          {TYPE_BADGE[node.nodeType]}
        </text>
      </g>
    );
  }

  const displayT1 = t1Suppliers.length > 0 ? t1Suppliers : [];
  const displayT2 = t2Suppliers.length > 0 ? t2Suppliers : [];

  return (
    <div className="min-h-screen bg-[#f5f6fa] p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl font-bold text-[#1a2436] mb-6">Value Chain Dynamic</h1>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div className="bg-white p-4 rounded-lg shadow">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Input JSON</h2>
            <textarea
              value={jsonInput}
              onChange={(e) => setJsonInput(e.target.value)}
              placeholder='{"company": "PT JAPFA", "tier1": [{"company": "Swift & Company", "tier2": [{"company": "Maple Leaf"}, {"company": "Nestle"}]}]}'
              rows={15}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg font-mono text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            {error && (
              <p className="mt-2 text-red-600 text-sm">{error}</p>
            )}
            {dbError && (
              <p className="mt-2 text-red-600 text-sm">DB Error: {dbError}</p>
            )}
            <div className="mt-3 flex gap-2">
              <button
                onClick={handleProcess}
                className="px-4 py-2 bg-[#1a2436] text-white rounded-lg hover:bg-[#2a3446] text-sm"
              >
                Process
              </button>
              <button
                onClick={() => loadFromDb('8ecd00b3-c4c2-4a59-a029-4adc60200a71')}
                disabled={dbLoading}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm disabled:opacity-50"
              >
                {dbLoading ? 'Loading...' : 'Load from DB'}
              </button>
            </div>
          </div>

          <div className="bg-white p-4 rounded-lg shadow">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Preview</h2>
            <div className="text-xs text-gray-600 space-y-1">
              <p><strong>Anchor:</strong> {anchorName}</p>
              <p><strong>Tier 1 Suppliers:</strong> {displayT1.length} (max {MAX_VISIBLE_T1})</p>
              <p><strong>Tier 2 Suppliers:</strong> {displayT2.length} (max {MAX_VISIBLE_T2} per tier1)</p>
              {dbLoaded && <p className="text-green-600">Loaded from DB</p>}
            </div>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow">
          <div className="flex items-center justify-between px-6 pt-5 pb-3">
            <h3 className="text-sm font-semibold text-gray-800">Value Chain Graph</h3>
            <button className="text-sm text-blue-600 hover:underline font-medium">View Data</button>
          </div>

          <div className="px-2 pb-2">
            <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block' }}>
              <text x={T2_CARD_LX + CW / 2} y={42} textAnchor="middle"
                fontSize={11} fontWeight="700" fontStyle="italic" fill="#9ca3af" pointerEvents="none">
                Tier 2 Suppliers
              </text>
              <text x={T1_CARD_LX + CW / 2} y={72} textAnchor="middle"
                fontSize={11} fontWeight="700" fontStyle="italic" fill="#9ca3af" pointerEvents="none">
                Tier 1 Suppliers
              </text>
              <text x={CUST_CARD_LX + CW / 2} y={42} textAnchor="middle"
                fontSize={11} fontWeight="700" fontStyle="italic" fill="#9ca3af" pointerEvents="none">
                Customers
              </text>

              {displayT2.map((node, i) => (
                <line key={`t2l-${node.id}`}
                  x1={T2_CARD_RX} y1={t2Positions.find(p => p.node.id === node.id)?.y ?? AY}
                  x2={T1_CARD_LX} y2={t1YMap[node.parentId || ''] ?? AY}
                  stroke={hovered === node.id ? '#2563eb' : '#e5e7eb'}
                  strokeWidth={hovered === node.id ? 1.5 : 1}
                  strokeDasharray="4 3"
                />
              ))}

              {displayT1.map((node, i) => (
                <line key={`t1l-${node.id}`}
                  x1={T1_CARD_RX} y1={t1Ys[i]}
                  x2={T1_HUB_X} y2={AY}
                  stroke={hovered === node.id ? '#2563eb' : '#d1d5db'}
                  strokeWidth={hovered === node.id ? 1.8 : 1}
                />
              ))}

              <line x1={T1_HUB_X + 6} y1={AY} x2={AX - AW / 2} y2={AY}
                stroke="#374151" strokeWidth={2.5} />

              <line x1={AX + AW / 2} y1={AY} x2={CUST_HUB_X - 6} y2={AY}
                stroke="#374151" strokeWidth={2.5} />

              {CUSTOMERS.map((node, i) => (
                <line key={`cl-${node.id}`}
                  x1={CUST_HUB_X} y1={AY}
                  x2={CUST_CARD_LX} y2={custYs[i]}
                  stroke={hovered === node.id ? '#2563eb' : '#d1d5db'}
                  strokeWidth={hovered === node.id ? 1.8 : 1}
                />
              ))}

              {displayT2.map((node) => {
                const pos = t2Positions.find(p => p.node.id === node.id);
                return pos ? renderCard(node, T2_CARD_LX, pos.y) : null;
              })}
              {displayT1.map((node, i) => renderCard(node, T1_CARD_LX, t1Ys[i]))}
              {CUSTOMERS.map((node, i) => renderCard(node, CUST_CARD_LX, custYs[i]))}

              <rect x={T1_HUB_X - 6} y={AY - 6} width={12} height={12} rx={2} fill="#374151" />
              <text x={T1_HUB_X} y={AY + 22} textAnchor="middle"
                fontSize={10} fontWeight="600" fill="#6b7280" pointerEvents="none">
                {displayT1.length} Suppliers
              </text>

              <rect x={CUST_HUB_X - 6} y={AY - 6} width={12} height={12} rx={2} fill="#374151" />
              <text x={CUST_HUB_X} y={AY + 22} textAnchor="middle"
                fontSize={10} fontWeight="600" fill="#6b7280" pointerEvents="none">
                {CUSTOMERS.length} Customers
              </text>

              <rect x={AX - AW / 2} y={AY - AH / 2} width={AW} height={AH} rx={6} fill="#1e3a6e" />
              <text x={AX} y={AY - 10} textAnchor="middle"
                fontSize={12} fontWeight="800" fill="white" pointerEvents="none">{anchorName.split(' ').slice(0, 2).join(' ')}</text>
              <text x={AX} y={AY + 6} textAnchor="middle"
                fontSize={12} fontWeight="800" fill="white" pointerEvents="none">{anchorName.split(' ').slice(2).join(' ')}</text>
              <text x={AX} y={AY + 23} textAnchor="middle"
                fontSize={9} fontWeight="600" fill="#93c5fd" pointerEvents="none">Anchor</text>

            </svg>
          </div>

          <div className="flex gap-8 px-6 pb-6 pt-3 border-t border-gray-100 text-xs text-gray-700">
            <div>
              <p className="font-bold mb-2 text-gray-900">Client Type</p>
              {(['Supplier', 'Local Customer', 'Global Customer'] as NodeType[]).map((type) => (
                <div key={type} className="flex items-center gap-2 mb-1.5">
                  <div className="relative w-10 h-4 rounded-sm overflow-hidden"
                    style={{
                      border: type === 'Global Customer'
                        ? '1px dashed #0f2d52'
                        : '1px solid #e5e7eb',
                    }}
                  >
                    <div className="absolute left-0 top-0 bottom-0 w-1"
                      style={{ backgroundColor: TYPE_COLOR[type] }} />
                  </div>
                  <span>{type}</span>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
